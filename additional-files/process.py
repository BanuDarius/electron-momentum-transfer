...
fig_mom, ax_mom = plt.subplots(figsize=(8, 6))
ax_mom.plot(x_emt, px_emt, 'k-', linewidth=1.5, label='EMT')
ax_mom.plot(x_smilei, px_smilei, 'r--', linewidth=1.5, label='SMILEI')
ax_mom.set_xlabel(r'Initial position $x/\lambda$', fontsize=15)
ax_mom.set_ylabel(r'Final momentum $p_x$ [a.u.]', fontsize=15)
ax_mom.set_xlim([-0.5, 0.5])
ax_mom.legend(loc='upper right', frameon=True)

fig_mom.tight_layout()
fig_mom.savefig("emt_vs_smilei_momentum.png", dpi=300)
plt.close(fig_mom)

fig_err, ax_err = plt.subplots(figsize=(8, 6))
ax_err.plot(x_emt, relative_error, 'b-', linewidth=1.2)
ax_err.set_xlabel(r'Initial position $x/\lambda$', fontsize=15)
ax_err.set_ylabel(r'Relative error $\epsilon$ [%]', fontsize=15)
ax_err.set_yscale('log')
ax_err.set_xlim([-0.5, 0.5])

fig_err.tight_layout()
fig_err.savefig("emt_vs_smilei_errors.png", dpi=300)
plt.close(fig_err)
